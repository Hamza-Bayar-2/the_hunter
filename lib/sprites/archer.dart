...
class ArcherPlayer extends SpriteAnimationGroupComponent
    with
        HasGameRef<MiniGame>,
        KeyboardHandler,
        TapCallbacks,
  ...
  late Bgm runSoundBmg =
      FlameAudio.bgmFactory(audioCache: FlameAudio.audioCache);
  final cameraShake = MoveEffect.by(Vector2(0, 15),
  ...
  Future<void> onLoad() async {
    hitbox = RectangleHitbox.relative(Vector2(0.25, 0.30),
  ...
  bool onKeyEvent(KeyEvent event, Set<LogicalKeyboardKey> keysPressed) {
    ...
    _keyBoardDirectionHandler(isUpKeyPressed, isDownKeyPressed,
        isRightKeyPressed, isLeftKeyPressed, isSpaceKeyPressed);
  ...
  void _keyBoardDirectionHandler(bool isUpKeyPressed, bool isDownKeyPressed,
      bool isRightKeyPressed, bool isLeftKeyPressed, bool isSpaceKeyPressed) {
  ...
  void _loadAnimation() {
    ...
    final attackAnimation = _spriteAnimation(
        archerState: "Attack", frameAmount: 6, stepTime: 0.085);
    final deathAnimation =
        _spriteAnimation(archerState: "Death", frameAmount: 10, stepTime: time);
    final fallAnimation =
        _spriteAnimation(archerState: "Fall", frameAmount: 2, stepTime: time);
    final getHitAnimation = _spriteAnimation(
        archerState: "Get Hit", frameAmount: 3, stepTime: time);
    final idleAnimation =
        _spriteAnimation(archerState: "Idle", frameAmount: 10, stepTime: time);
    final jumpAnimation =
        _spriteAnimation(archerState: "Jump", frameAmount: 2, stepTime: time);
    final runAnimation =
        _spriteAnimation(archerState: "Run", frameAmount: 8, stepTime: time);
    final lastFrameDeath = _spriteAnimation(
        archerState: "DeathStatic", frameAmount: 1, stepTime: 10);
  ...
  void _archerMovement(double dt) {
    ...
    position.clamp(
        Vector2(
            size.x - (size.x / 1.2), gameRef.background.size.y * 0.6 - size.y),
        Vector2(gameRef.background.size.x, gameRef.background.size.y) -
            (size / 6));
  }
  ...
  void _archerRunningSound() {
    if (current != ArcherState.idle &&
      ...
      isArcherRunning = true;
        ...
        current == ArcherState.attack ||
        gameRef.miniGameBloc.state.flutterPage != 1) {
  ...
  void _updateArcherDecorator(double dt) {
    ...
    if (isArcherHealthIncreased) {
      ...
      decoratorForArcher.replaceLast(
          PaintDecorator.tint(const Color.fromARGB(143, 92, 255, 92)));
      ...
      if (isLowHealth) {
        decoratorForArcher.replaceLast(
            PaintDecorator.tint(const Color.fromARGB(93, 255, 0, 0)));
      } else {
      ...
      archerHelathIncreaseCountdown.stop();
        ...
        !gameRef.miniGameBloc.state.isArcherDead) {
